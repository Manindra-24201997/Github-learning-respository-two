({
	doInIt : function(component, event, helper) {
        ...
        component.set('v.columns', [
            {label: 'Job#', fieldName: 'JobSuite__Job_Auto_Number__c', type: 'text',sortable: true},
            ...
            {label: 'Status', fieldName: 'JobSuite__Status__c', type: 'text',sortable: true},
            {label: 'Job Description', fieldName: 'JobSuite__Job_Description__c', type: 'text',sortable: true},
            ...
            {label: 'Start Date', fieldName: 'JobSuite__Start_Date__c', type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Start Date' }},
            {label: 'Due Date', fieldName: 'JobSuite__Due_Date__c', type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Due Date' }},
            {label: 'Completion Date', fieldName: 'JobSuite__Completion_Date__c', type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Completion Date'}},
    ...
    loadMoreData : function(component, event, helper) {
     ...
     navigateToJobSearchResult : function(component, event, helper) {
        window.open('/one/one.app#/alohaRedirect/apex/JobSuite__Job_Search_Result_InPDF','_blank'); 
    },
    ...
    navigateToMyComponent : function(component, event, helper) {
       var navService = component.find("navService");
        var pageReference = {
            ...
            attributes: {
                objectApiName: 'JobSuite__Job__c',
                actionName: 'new'
            }
        };
        ...
        JobsAction.setCallback(this, function(response){ 
            ...
            if (response.getState() === "SUCCESS") {
                ...
                for (var i = 0; i < rows.length; i++) {
                    var row = rows[i];
                    row.JobLink = '/'+row.Id;
                    if(row.JobSuite__JS_Client__r!=null ){
                        row.ClientLink='/'+row.JobSuite__JS_Client__r.Id;
                        row.ClientName=row.JobSuite__JS_Client__r.Name;  
                    }
                    if(row.JobSuite__Campaign__r!=null){
                        row.CampLink='/'+row.JobSuite__Campaign__r.Id;
                        row.CampName=row.JobSuite__Campaign__r.Name;  
                    }
                }
