({
    loadDataToCalendar :function(component,data){  
        ...
        $('#'+dateval).fullCalendar({
            ...
            eventMouseover : function(event, jsEvent, view) {
                ...
                if(job){
                    var startdatesplit = startdate.split('-'); //Newly added for format
                    var duedateformatsplit = duedate.split('-') ;//Newly added for format
                    var tooltip = '<div class="tooltipevent" style="box-shadow: 4px 4px 4px 4px '+event.color+';padding:10px;background-color:'+event.color+';color:white;position:absolute;z-index:10001;">' +
                       ...
                       /* '<br>' + '<B>Start Date: </B>' + startdate +*/
                         '<br>' + '<B>Start Date: </B>' + startdatesplit[1]+'-'+startdatesplit[2]+'-'+startdatesplit[0]+//Newly added for format
                       /* '<br>' + '<B>Due Date: </B>' + duedate +*/
                         '<br>' + '<B>Due Date: </B>' + duedateformatsplit[1]+'-'+duedateformatsplit[2]+'-'+duedateformatsplit[0]+//Newly added for format
                        '</div>';
                }
                
                if(camp){
                    var startdate =  event.start.format();//Newly added for format
                    var startdatesplit = startdate.split('-'); //Newly added for format
                    var enddate=event.end.format();
                    duedate = moment(enddate).subtract(1, 'days').format();
                    duedate = duedate.split('T')[0];
                    var duedateformat = duedate.split('T')[0];  //Newly added for format
                     var duedateformatsplit = duedateformat.split('-') ;//Newly added for format
                    
                    var tooltip = '<div class="tooltipevent" style="box-shadow: 4px 4px 4px 4px '+event.color+';padding:10px;background-color:'+event.color+';color:white;position:absolute;z-index:10001;">' +
                        '<B>Name: </B>' + event.title +
                      /*  '<br>' + '<B>Start Date: </B>' + ftdate + */ // commeneted for date format
                        '<br>' + '<B>Start Date: </B>' + startdatesplit[1]+'-'+startdatesplit[2]+'-'+startdatesplit[0]+
                       /* '<br>' + '<B>End Date: </B>' +duedate +*/ // commeneted for date format
                          '<br>' + '<B>End Date: </B>' + duedateformatsplit[1]+'-'+duedateformatsplit[2]+'-'+duedateformatsplit[0]+  
                        '<br>' + '<B>Description: </B>' +event.CampDesc +
                        '</div>';
                }
                
                if(task){
                     var duedateformat = event.start.format();//Newly added for format
                      var duedateformatsplit = duedateformat.split('-') ;//Newly added for format
                    var tooltip = '<div class="tooltipevent" style="box-shadow: 4px 4px 4px 4px '+event.color+';padding:10px;background-color:'+event.color+';color:white;position:absolute;z-index:10001;">' +
    ...
    formatFullCalendarData : function(component,events) {
        ...
        try{
            ...
            //  console.log('>>>>>>>Task Events Length >>>>>>>>>>>>>>'+events.JobSuite__Job_Task__r.length);
            for(var i = 0;i < events.length;i++){
                
                /*console.log('>>>>>>>events>>>>>>>>>>'+JSON.stringify(events[i]));
                console.log('>>>>>>>Tasks'+JSON.stringify(events[i].JobSuite__Job_Task__r));
                console.log('>>>>>>>1startdate'+events[i].JobSuite__Start_Date__c);
                console.log('>>>>>>>1End Date'+events[i].JobSuite__Due_Date__c);*/
                
                if(job){
                    
                    //console.log('>>>>Job if starts>>>>'+job);
                    
                    if((events[i].JobSuite__Start_Date__c!=null && (events[i].JobSuite__Due_Date__c!=null || events[i].JobSuite__Due_Date__c!=NaN))){
                        //console.log('>>>>>>>Job Start end 1>>>>>>>>');
                        startdate = $A.localizationService.formatDate(events[i].JobSuite__Start_Date__c);
                        endate = $A.localizationService.formatDate(events[i].JobSuite__Due_Date__c);
                    ...
                    if((events[i].JobSuite__Start_Date__c!=null && (events[i].JobSuite__Due_Date__c==null || events[i].JobSuite__Due_Date__c==NaN))){
                        ...
                        startdate = $A.localizationService.formatDate(events[i].JobSuite__Start_Date__c);
                        enddate =  $A.localizationService.formatDate(events[i].JobSuite__Start_Date__c);
                    }
                    
                    if((events[i].JobSuite__Start_Date__c==null && (events[i].JobSuite__Due_Date__c!=null || events[i].JobSuite__Due_Date__c!=NaN))){
                        //console.log('>>>>>>>Job Start end 3>>>>>>>>');
                        startdate =$A.localizationService.formatDate(events[i].JobSuite__Due_Date__c);
                        enddate = $A.localizationService.formatDate(events[i].JobSuite__Due_Date__c);
                    }
                    
                    if(events[i].JobSuite__JS_Client__r!=null){
                        Clnt=events[i].JobSuite__JS_Client__r.Name;
                    }
                    if(events[i].JobSuite__Campaign__r!=null){
                        camp=events[i].JobSuite__Campaign__r.Name;
                    }
                    if(events[i].JobSuite__Schedule_Calc__c!=null){
                        SchCal=events[i].JobSuite__Schedule_Calc__c;
                    }
                    
                    title=events[i].Name +' - '+ events[i].JobSuite__Job_Auto_Number__c;
                ...
                if(campai){
                    ...
                    title=events[i].Name;
                    if(events[i].JobSuite__Start_Date__c!=null){
                        startdate=$A.localizationService.formatDate(events[i].JobSuite__Start_Date__c);
                    }
                    if(events[i].JobSuite__End_Date__c!=null){
                        // console.log('>>>>>>>Cmp end >>>>>>>>');
                        endate=$A.localizationService.formatDate(events[i].JobSuite__End_Date__c);
                        enddate = moment(endate).add(1, 'days');
                    }
                    if(events[i].JobSuite__Campaign_Description__c!=null){
                        Cdesc=events[i].JobSuite__Campaign_Description__c;
                    }
                    else{
                        Cdesc ='';
                    }
                ...
                if(task){
                    ...
                    //console.log('>>>>>>>Task Events Length >>>>>>>>>>>>>>'+events[i].JobSuite__Job_Task__r.length);
                    
                    for(var j = 0;j < events[i].JobSuite__Job_Task__r.length;j++)
                    {
                        
                        //console.log('>>>>>>>inner for loop>>>>>>>>'+JSON.stringify(events[i].JobSuite__Job_Task__r[j].JobSuite__Revised_Due_Date__c));
                        
                        tskid=events[i].JobSuite__Job_Task__r[j].Id;
                        tsktitle=events[i].JobSuite__Job_Task__r[j].Name +' - '+ events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__Job_Auto_Number__c;
                        
                        if(events[i].JobSuite__Job_Task__r[j].JobSuite__Revised_Due_Date__c!=null)
                        {
                            tskstartdate=$A.localizationService.formatDate(events[i].JobSuite__Job_Task__r[j].JobSuite__Revised_Due_Date__c);
                            tskenddate=$A.localizationService.formatDate(events[i].JobSuite__Job_Task__r[j].JobSuite__Revised_Due_Date__c);
                        }
                        
                        if(events[i].JobSuite__Job_Task__r[j].JobSuite__Assigned_Users__c!=null){
                            tskStaff=events[i].JobSuite__Job_Task__r[j].JobSuite__Assigned_Users__c;
                        } 
                        else{
                            tskStaff='';
                        }
                        
                        tskjob=events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.Name +' - '+ events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__Job_Auto_Number__c;
                        
                        if(events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__Campaign__c!=null && events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__Campaign__c!=undefined){        
                            tskjobCamp=events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__Campaign__r.Name;
                        }
                        else{
                            tskjobCamp='';
                        }
                        if(events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__JS_Client__c!=null && events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__JS_Client__c!=undefined){
                            
                            tskjobClient=events[i].JobSuite__Job_Task__r[j].JobSuite__Job__r.JobSuite__JS_Client__r.Name;
                        }
                        else{
                            tskjobClient='';
                        }
    ...
    fetchCampCalenderEvents : function(component, event, helper) {
        try{
            ...
            //Newly added for camp types
            var camptypes = component.get("v.CamptypesVal");
            
            //Newly added for campaign multi select picklist values
              var campBuvalues = component.get("v.CampBu");
    ...
    fetchjobEvents : function(component, event, helper) {
        try{
            ...
            var BUVal = component.get("v.JobBu");
            // console.log('>BUVal>>>>>>>'+BUVal);
