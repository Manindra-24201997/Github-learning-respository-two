({
    ...
    onloadrecord : function(component, event, helper) {
        ...
        if(recId!=null){
            ...
            helper.getVendor(component, event, helper);
            helper.getVendorCon(component, event, helper,POfilds.record.fields.JobSuite__Vendor__c.value);
            helper.getStaff(component, event, helper);
            component.set("v.PORecordDetails",POfilds.record.fields);
            console.log('>>>>>>>>>>>POfilds  22222>>>>>>>>>>>>>'+JSON.stringify(POfilds.record.fields));
            console.log('>>>>>>>>>>>POfilds 22222>>>>>>>>>>>>>'+JSON.stringify(POfilds.record.fields.JobSuite__Vendor__r.value.fields.Name.value));
            setTimeout($A.getCallback(function() {
                if(recId==null || recId=='undefined'){
                    //component.find("VendorId").set("v.value",POfilds.record.fields.JobSuite__Vendor__c.value);
                    //console.log('==Vendor__c Onload Id=='+component.find("VendorId").get("v.value"));
                }
            }), 500);
            
            setTimeout($A.getCallback(function() {
                component.find("VenConId").set("v.value",POfilds.record.fields.JobSuite__Vendor_Contact__c.value);
                console.log('==Vendor_Contact__c Onload Id=='+component.find("VenConId").get("v.value"));
            }), 500);
            
            setTimeout($A.getCallback(function() {
                component.find("StaffID").set("v.value",POfilds.record.fields.JobSuite__Approved_By_Staff__c.value);
    ...
    RecordSubmit: function(component, event, helper) {
        ...
        POfields["JobSuite__Job__c"]=component.get("v.jobrecordId");
        var missField = "";
        var missField1 = "";
        var missField2 = "";
        var reduceReutrn = component.find('JobPOField').reduce(function(validFields, inputCmp) {   
            if(inputCmp.get("v.fieldName") == "JobSuite__Vendor__c"){
                var VenName = inputCmp.get("v.value"); 
                if (VenName == null || VenName == '') 
                    missField = "Vendor";
            }
            
            else if(inputCmp.get("v.fieldName") == "JobSuite__Issue_Date__c" ){
                ...
                else if(inputCmp.get("v.fieldName") == "JobSuite__Due_Date__c"){
            ...
            event.preventDefault();
                ...
                event.preventDefault();
                   ...
                   /* if(component.get('v.PORecordid')==null || component.get('v.PORecordid')=='undefined'){
                        POfields["JobSuite__Vendor__c"]=component.find("VendorId").get("v.value");
                        console.log('===Ven==='+component.find("VendorId").get("v.value"));
                    }*/
                    
                    POfields["JobSuite__Vendor_Contact__c"]=component.find("VenConId").get("v.value");
                    POfields["JobSuite__Approved_By_Staff__c"]=component.find("StaffID").get("v.value");
