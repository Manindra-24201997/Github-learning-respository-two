({
    ...
    onloadrecord : function(component, event, helper) {
        ...
        if(recId!=null){
            ...
            //helper.getGLDepartment(component, event, helper);
            component.find("GLDSelectId").set("v.value",Estfilds.record.fields.JobSuite__GL_Department__c.value);
            
            var GLCodeVal=Estfilds.record.fields.JobSuite__GL_Code__c.value;
            var VendorVal=Estfilds.record.fields.JobSuite__Vendor__c.value;
            
            helper.getGLCode(component, event,helper, GLCodeVal, VendorVal);
          
          /*  setTimeout($A.getCallback(function() {
                component.find("GLCodeSelectId").set("v.value",Estfilds.record.fields.JobSuite__GL_Code__c.value);
                console.log('==GL_Code__c Onload Id=='+component.find("GLCodeSelectId").get("v.value"));
                helper.getVendor(component, event, helper);
            }), 500);
           
            setTimeout($A.getCallback(function() {
                component.find("VendorId").set("v.value",Estfilds.record.fields.JobSuite__Vendor__c.value);
    ...
    RecordSubmit: function(component, event, helper) {
        ...
        var reduceReutrn = component.find('JobEstField').reduce(function(validFields, inputCmp) {   
            if(inputCmp.get("v.fieldName") == "Name" ){
                ...
                else if(inputCmp.get("v.fieldName") == "JobSuite__Amount__c"){
                    ...
                    event.preventDefault();   
                        ...
                        event.preventDefault();
                            ...
                            event.preventDefault();
                                ...
                                Estfields["JobSuite__GL_Department__c"]=component.find("GLDSelectId").get("v.value");
                                //console.log('===GL_Department__c==='+component.find("GLDSelectId").get("v.value"));
                                Estfields["JobSuite__GL_Code__c"]=component.find("GLCodeSelectId").get("v.value");
                                //console.log('===Code==='+component.find("GLCodeSelectId").get("v.value"));
                                Estfields["JobSuite__Vendor__c"]=component.find("VendorId").get("v.value");
