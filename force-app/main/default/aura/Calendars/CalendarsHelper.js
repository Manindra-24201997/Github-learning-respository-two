({
    ...
    FetchLabels : function(component, event, helper) {
        ...
        FieldLabels.setParams({
            ObjNames:['JobSuite__Campaign__c','JobSuite__Job__c','JobSuite__Job_Task__c']
        });
    ...
    FetchJobCalcriteriaVal : function(component, event, helper) {
        ...
        /* Multi picklist for job */
       
        var pickval = [];
         var BuVal = component.get("c.fetchJobBuPicklistval");
        BuVal.setCallback(this, function(BuValRes) {
            var BuValstate = BuValRes.getState();
            //alert('>>>>>>>>>>>Job status picstate>>>>>>>>>>>>>>>'+BuValRes.getReturnValue());
            if (BuValstate === "SUCCESS") {
                 var vals = BuValRes.getReturnValue();
                for(var i = 0 ; i <vals.length ; i++){
                     var Colorobj= {};
                     Colorobj['Id'] = vals[i];
                    Colorobj['Name'] = vals[i];
                     Colorobj['flag'] = false;
                        // Colorobj=[{"Id":vals[i],"Name":vals[i],"flag":false}];
                    pickval.push(Colorobj);
                }
               // alert(JSON.stringify(pickval));
                component.set("v.ColorList", pickval);
            }
        });
        $A.enqueueAction(BuVal);
    },
    ...
    FetchCampCalcriteriaVal : function(component, event, helper) {
        ...
        CampStatusVal.setCallback(this, function(CampStatusValRes) {
            ...
            if (CampStatusValstate === "SUCCESS") {
                var campfilters = CampStatusValRes.getReturnValue();
                component.set("v.CampstatusPicklist",campfilters.statusfilters);
                 component.set("v.CamptypesPicklist",campfilters.campaigntypefilters); 
                //The Below code is for multi select picklist code for BUsiness unit in campaign.
                var vals = campfilters.campaignBUfilters;
                //alert("vals "+vals);
                for(var i = 0 ; i <vals.length ; i++){
                     var Colorobj= {};
                     Colorobj['Id'] = vals[i];
               ...
               // alert(JSON.stringify(pickval));
                component.set("v.ColorList2", pickval); 
            }
        });
    ...
    FetchCampName:  function(component, event, helper) {
        ...
        //Newly added below line for camp types
         var CamptypesSel=component.get("v.SelectedCamptypes");
       // alert(CamptypesSel);
        
        var CampNameVal = component.get("c.getCampaignNames");
        CampNameVal.setParams({
            status:CampstatusSel,
            cmptypes:CamptypesSel
        });
