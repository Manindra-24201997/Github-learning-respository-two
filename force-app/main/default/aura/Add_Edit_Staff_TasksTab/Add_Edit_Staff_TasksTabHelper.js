({
        ...
        AStaffAction.setCallback(this, function(response){
            ...
            if (response.getState() === "SUCCESS") {
                if(response.getReturnValue() === "OK"){
                    ...
                    evt.fire();*/
                    //var stf = component.get("v.stfnamecmp");
                    var compEvent = component.getEvent("ComponentEventTasks");
                    compEvent.setParams({
                        "stfname" : 'Component event invoked'
                    });
                    compEvent.fire();                   
                }
                else{
    ...
    navigatetothis : function(component,event,helper){
        var evt = $A.get("e.force:navigateToComponent");
        evt.setParams({
            componentDef : "c:Add_Edit_Staff_TasksTab",
        });
        evt.fire();
    },
