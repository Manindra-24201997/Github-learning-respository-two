({
    ...
    doInit: function(component, event, helper) {
        ...
        setTimeout($A.getCallback(function (){
            if(component.get("v.JobDetailAccessible[0]")){
                ...
                var ToastMsgDisplayAppSteps = $A.get("e.force:showToast");
                ToastMsgDisplayAppSteps.setParams({
                    "title": "ERROR",
                    "type": "error",
                    "message": 'Approval Steps has insufficient access to View'                        
                });
                ToastMsgDisplayAppSteps.fire();
            }
        }), 1500);
    ...
    handleDeleteApprovalSchedule : function(component, event, helper){
        ...
        if(component.get("v.JobDetailAccessible[3]")){
            ...
            var ToastMsgDeleteAppSteps = $A.get("e.force:showToast");
            ToastMsgDeleteAppSteps.setParams({
                "title": "ERROR",
                "type": "error",
                "message": 'Error, Approval Process has insufficient access to Delete'                        
            });
            ToastMsgDeleteAppSteps.fire();
        }
    ...
    handlecreateApprovalSchedule : function(component, event, helper){
        if(component.get("v.JobDetailAccessible[1]")){
            ...
            var ToastMsgCreateAppSteps = $A.get("e.force:showToast");
            ToastMsgCreateAppSteps.setParams({
                "title": "ERROR",
                "type": "error",
                "message": 'Error, Approval Steps has insufficient access to create'                        
            });
            ToastMsgCreateAppSteps.fire();
        }
    },
        ...
        if(FileId!='' && AppSchedTempId!='')
        {
            ...
            if(component.get("v.JobDetailAccessible[1]")){
                ...
                helper.showToast({
                    "title": "Error!!",
                    "type": "error",
                    "message": 'Error, Approval Step have insufficient access to create'
                }); 
            }
    ...
    TaskOnsubmit : function(component, event, helper){
        ...
        console.log('===TaskOnsubmit fields=='+JSON.stringify(fields));
        ...
        /* else if(StausField=="Status"){
            ...
            event.preventDefault();
                ...
                console.log('===TaskOnsubmit fields last=='+JSON.stringify(fields));
                component.find("TaskEditForm").submit(fields);
            }
    },
    
    TaskOnSuccess : function(component, event, helper){
        console.log('>>>>>>>>>>>TaskOnSuccess>>>>>>>>>>>>>>>>>>');
        var TaskId=component.get("v.EditTaskId");
        console.log('>>>>>>>>>>>TaskOnSuccess TaskId>>>>>>>>>>>>>>>>>>',TaskId);
        var UpdateRestDays=component.get("v.isDaysChanged");
        console.log('>>>>>>>>>>>TaskOnSuccess UpdateRestDays>>>>>>>>>>>>>>>>>>',UpdateRestDays);
        var UpdateRestDates=component.get("v.isDueDateChanged");
        console.log('>>>>>>>>>>>TaskOnSuccess UpdateRestDates>>>>>>>>>>>>>>>>>>',UpdateRestDates);
    ...
    TaskAboveOnsubmit : function(component, event, helper){
        ...
        var fields = event.getParam("fields");
        fields["Task__c"]=component.get("v.recordId");
        ...
        if(NameField=="Name"){
            ...
            Aboveaction.setCallback(this, function(AboveResponse){
                ...
                if (AboveResponse.getState() === "SUCCESS") {
                    ...
                    fields["Created_From_Job_Task__c"]=true;
    ...
    insertatEnd : function(component, event, helper) {
        if(component.get("v.JobDetailAccessible[1]")){
            ...
            helper.showToast({
                "title":"Error!!",
                "type": "error",
                "message": 'Approval Process has insufficient access to create'
            }); 
        }
    },
        ...
        else if(BFileName=='' || BFileName == null)
        {
            ...
            //event.preventDefault();
                ...
                Belowaction.setParams({
                    jobTskid : component.get("v.recordId"),
                    days :fields["Days__c"],
        ...
        switch (action.name) {
            ...
            case 'Submit_Action':
                if(component.get("v.JobDetailAccessible[2]")){
                    ...
                    var ToastMsgSubmitAppSteps = $A.get("e.force:showToast");
                    ToastMsgSubmitAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Steps has insufficient access to Update'                        
                    });
                    ToastMsgSubmitAppSteps.fire();
                }
            ...
            case 'reSubmit_Action':
                if(component.get("v.JobDetailAccessible[2]")){
                    ...
                    var ToastMsgReSubmitAppSteps = $A.get("e.force:showToast");
                    ToastMsgReSubmitAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Steps has insufficient access to Update'                        
                    });
                    ToastMsgReSubmitAppSteps.fire();
                }
            ...
            case 'Undo':
                if(component.get("v.JobDetailAccessible[2]")){
                    ...
                    var ToastMsgUndoAppSteps = $A.get("e.force:showToast");
                    ToastMsgUndoAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Steps has insufficient access to Update'                        
                    });
                    ToastMsgUndoAppSteps.fire();
                }
            ...
            case 'Task_Edit':
                if(component.get("v.JobDetailAccessible[2]")){
                    ...
                    var ToastMsgEditAppSteps = $A.get("e.force:showToast");
                    ToastMsgEditAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Steps has insufficient access to Update'                        
                    });
                    ToastMsgEditAppSteps.fire();
                }
                break;
            ...
            case 'Task_AddEdit':
                if(component.get("v.JobDetailAccessible[4]")){
                    ...
                    var ToastMsgAddEditAppSteps = $A.get("e.force:showToast");
                    ToastMsgAddEditAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Step role has insufficient access to edit/update'                        
                    });
                    ToastMsgAddEditAppSteps.fire();
                }
                break;
            ...
            case 'Task_Delete':
                if(component.get("v.JobDetailAccessible[3]")){
                    ...
                    var ToastMsgDelAppSteps = $A.get("e.force:showToast");
                    ToastMsgDelAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Step has insufficient access to delete'                        
                    });
                    ToastMsgDelAppSteps.fire();
                }
                break;   
            ...
            case 'Task_Above':
                if(component.get("v.JobDetailAccessible[1]")){
                    ...
                    var ToastMsgtskAboveAppSteps = $A.get("e.force:showToast");
                    ToastMsgtskAboveAppSteps.setParams({
                        "title": "ERROR",
                        "type": "error",
                        "message": 'Error, Approval Process has insufficient access to Create'                        
                    });
                    ToastMsgtskAboveAppSteps.fire();
                }
                break;
