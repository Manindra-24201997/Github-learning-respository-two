    ...
    fetchFieldLabels : function(component, event , helper){
        ...
        action.setCallback(this, function(res){
            if(res.getState() === "SUCCESS"){
                ...
                console.log('===JobSuite__Estimate_Spec_Item__c====='+JSON.stringify(JSON.parse(res.getReturnValue())));
                //component.set("v.ObjectType",JSON.parse(res.getReturnValue()));
                
                component.set("v.columns",[
                    { label: tableHeaders.JobSuite__Job__c.JobSuite__Job_Auto_Number__c.label, fieldName: 'JobLink', sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'jobNo' }, target: '_self', tooltip:{ fieldName: 'jobNo' }}},
                    { label: tableHeaders.JobSuite__Job__c.Name.label, fieldName: 'JobLinkName', sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'jobName' }, target: '_self' , tooltip:{ fieldName: 'jobName' }}},
                    { label: tableHeaders.JobSuite__Job__c.JobSuite__JS_Client__c.label, fieldName: 'clientDisplay', type: 'text', sortable: true },
                    { label: tableHeaders.JobSuite__Job__c.JobSuite__Due_Date__c.label, fieldName: 'jobDuedate', type: 'date-local', sortable: true, cellAttributes: { iconName: 'utility:event', iconAlternativeText: tableHeaders.JobSuite__Job__c.JobSuite__Due_Date__c.label}},
