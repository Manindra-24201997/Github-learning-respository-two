({
    doInIt : function(component, event, helper) {
        ...
        component.set('v.columns', [
            ...
            {label: 'Location', fieldName: 'JobSuite__Location__c', type: 'text', sortable: true },
            {label: 'Expiration Date', fieldName: 'JobSuite__Expiration_Date__c', type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Expiration Date' }},
    ...
    navigateToMyComponent : function(component, event, helper) {
        ...
        var pageReference = {
            ...
            attributes: {
                objectApiName: 'JobSuite__Billboard__c',
                actionName: 'new'
            }
        };
        ...
        switch (action.name) {
            case 'Edit_BB':
                ...
                evt.setParams({
                    ...
                    componentAttributes: {
                        sObjectName : "JobSuite__Billboard__c",
                        recordId :row.Id
                    }
                });
                evt.fire();
        ...
        BBAction.setCallback(this, function(response){ 
            ...
            if (response.getState() === "SUCCESS") {
                ...
                for (var i = 0; i < rows.length; i++) {
                    var row = rows[i];
                    row.BBLink = '/'+row.Id;
                    if(row.JobSuite__Client_Name__c!=null ){
                        row.ClientLink='/'+row.JobSuite__Client_Name__r.Id;
                        row.ClientName=row.JobSuite__Client_Name__r.Name;  
                    }
                    if(row.JobSuite__BB_Vendor__c!=null){
                        row.VenLink='/'+row.JobSuite__BB_Vendor__r.Id;
                        row.VenName=row.JobSuite__BB_Vendor__r.Name;  
                    }
                }
