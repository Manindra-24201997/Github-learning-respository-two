({
    fetchFieldLabels : function(component, event , helper){
        ...
        action.setParams({
            ObjNames:['JobSuite__Job_Task__c','JobSuite__Job__c']
        });
        
        action.setCallback(this, function(res){
            if(res.getState() === "SUCCESS"){
                var tableHeaders=JSON.parse(res.getReturnValue());
                console.log('===JobSuite__Job_Task__c and JOb====='+JSON.stringify(JSON.parse(res.getReturnValue())));
                //component.set("v.ObjectType",JSON.parse(res.getReturnValue()));
                
                component.set("v.columns",[
                    // { label: 'Color', fieldName: 'Color'},  
                    { type: 'action', typeAttributes: {rowActions: rowActions}},
                    { label: tableHeaders.JobSuite__Job_Task__c.JobSuite__Job__c.label, fieldName: 'JobId', editable: true, sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'JobName' }, target: '_self' , tooltip:{ fieldName: 'JobName' }}},
                    { label: tableHeaders.JobSuite__Job_Task__c.Name.label, fieldName: 'TaskId', sortable: true, type: 'url',cellAttributes: {class: { fieldName: "TaskColorcode" } }, typeAttributes: { label: { fieldName: 'TaskName' }, target: '_self', tooltip:{ fieldName: 'TaskName' }}},
                    { label: tableHeaders.JobSuite__Job_Task__c.JobSuite__Revised_Due_Date__c.label, fieldName: 'TaskDue', type: 'date-local', sortable: true, cellAttributes: { iconName: 'utility:event', iconAlternativeText: tableHeaders.JobSuite__Job_Task__c.JobSuite__Revised_Due_Date__c.label} },
                    { label: tableHeaders.JobSuite__Job__c.JobSuite__Job_Auto_Number__c.label, fieldName: 'JobIdNo', sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'JobNum' }, target: '_self', tooltip:{ fieldName: 'JobNum' }}}
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                for (var i = 0; i < rows.length; i++) {
                    ...
                    row.Color= row.color;
                    row.JobId= '/'+row.jobTaskrole.JobSuite__Job_Task__r.JobSuite__Job__r.Id;
                    row.JobIdNo= '/'+row.jobTaskrole.JobSuite__Job_Task__r.JobSuite__Job__r.Id;
                    row.TaskId= '/'+row.jobTaskrole.JobSuite__Job_Task__r.Id;
                    
                    row.JobName=row.jobTaskrole.JobSuite__Job_Task__r.JobSuite__Job__r.Name;
                    /* if(row.color=='Green')
                    {
                      row.TaskName=row.jobTaskrole.JobSuite__Job_Task__r.Name;
                      $A.util.addClass(row.TaskName, 'textClassGreen');
                    }*/
                    row.TaskColorcode=row.color+'_color';
                    row.TaskName=row.jobTaskrole.JobSuite__Job_Task__r.Name;
                    row.TaskDue=row.jobTaskrole.JobSuite__Job_Task__r.JobSuite__Revised_Due_Date__c;
                    row.JobNum=row.jobTaskrole.JobSuite__Job_Task__r.JobSuite__Job__r.JobSuite__Job_Auto_Number__c;
                }
    ...
    TaskDone: function(component, event, helper, selectTskId){
       var taskview=component.find("TaskVId").get("v.value");
        ...
        if(selectTskId!=null || selectTskId!=undefined){
            ...
            TSKDone.setParams({
                strJTID :row.jobTaskrole.JobSuite__Job_Task__r.Id
            });
