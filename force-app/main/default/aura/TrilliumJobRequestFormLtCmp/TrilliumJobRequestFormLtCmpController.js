({
    ...
    hideSpinner : function(component,event,helper){
     ...
     ShowMaterialItemList : function(component, event, helper) {
        ...
        for(var i=0;i<clientContacts.length;i++){
            if(clientContacts[i].Contactselect==true){
                selectClient=selectClient+1;
                clientContactSelectList.push(clientContacts[i].ClientConWrap);
            }
        }
        if(selectClient>0){
    ...
    onMiInput : function(component, event, helper) {      
     ...
     goToCobineList : function(component, event, helper){
        ...
        for(var i=0;i<MaterialItems.length;i++){
            if(MaterialItems[i].MISelect==true){
                count=count+1;
    ...
    checkQuantity : function(component, event, helper){
        ...
        var divQtyid=event.getSource().get("v.labelClass");           
    ...
    editAdress : function (component, event, helper){       
        var button=event.getSource().get("v.name");  
        console.log('--button---'+button);
        document.getElementById(button+'noEdit').setAttribute('style','display:none'); 
        document.getElementById(button+'inEdit').setAttribute('style','display:'); 
        var combinedWrapper=component.get("v.combineStoreWrapper");
        for(var i=0;i<combinedWrapper.length;i++){
            combinedWrapper[button].editmode=true;
        }
        console.log('-combineStoreWrapper-editmode--'+JSON.stringify(combinedWrapper));
        component.set("v.combineStoreWrapper",combinedWrapper);
    },
    submitOrder : function(component, event, helper){
        ...
        var combinedWrapper=component.get("v.combineStoreWrapper");
        var selectedMaterialItems=[];
        //console.log('-combineStoreWrapper---'+JSON.stringify(combinedWrapper));
        ...
        var BillingInstruField = component.find("BillingInstru");
        ...
        for(var i=0;i<BillingInstruField.length;i++){
            ...
            if($A.util.isEmpty(BillingInstruFieldValue)){
                component.set("v.isSpinner", false);
        ...
        if(emailcount>0 && billcount>0){
          ...
          for(var i=0;i<MaterialItems.length;i++){
            if(MaterialItems[i].MISelect==true){
                selectedMaterialItems.push(MaterialItems[i].MIWrap);
            }
        }
             
        helper.placeOrder(component, event, helper,combinedWrapper,selectedMaterialItems);  
        }
