        ...
        action.setCallback(this,function(res){
            ...
            if(res.getState()==="SUCCESS"){
                ...
                for(var i=0;i<rows.length;i++){
                    var row=rows[i];
                    if(row.JobSuite__User__r){
                        row.TSUserName=row.JobSuite__User__r.Name;
                    }
    ...
    getAllProgressTasks :function(component,event,helper,taskFilter,campFilter,sTempFilter,StaffFilter,roleFilter){
        ...
        action.setCallback(this,function(res){
            ...
            if(res.getState()==="SUCCESS"){
                ...
                for(var i=0;i<rows.length;i++){
                    var row=rows[i];
                    row.JobNo=row.JobSuite__Job__r.JobSuite__Job_Auto_Number__c;
                    row.JobName=row.JobSuite__Job__r.Name;
                    row.JobUrl='/'+row.JobSuite__Job__c;
