({
    afterScriptsLoaded : function(component, event, helper) {
        ...
        component.set("v.columns",[
            ...
            {label:'Start Date', fieldName:'JobSuite__Start_Date__c', type:'text',sortable:true,type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Start Date' }},
            {label:'Due Date', fieldName:'JobSuite__Revised_Due_Date__c', type:'text',sortable:true,type: 'date-local',editable: true,sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Due Date' }},
            {type: 'action', typeAttributes: { rowActions: rowActions }} 
        ]);
        component.set("v.tscolumns",[
            //{label:'Task', fieldName:'JobSuite__Task_Name__c', type:'text',sortable:true},
            {label:'Staff', fieldName:'TSUserName', type:'text',sortable:true},
            {label:'Date', fieldName:'JobSuite__Date__c', type:'text',sortable:true,type: 'date-local',sortable: true,cellAttributes: { iconName: 'utility:event', iconAlternativeText: 'Date' }},
            {label:'Hours Worked', fieldName:'JobSuite__Hours_Worked__c', type:'number',sortable:true,cellAttributes: {alignment: 'left'}},
    ...
    NoAction : function(component, event, helper){
        ...
        var dueDate=editedRecord[0].JobSuite__Revised_Due_Date__c
    ...
    rowselection : function(component, event, helper) {
        ...
        //console.log('==Job ID===='+selRow.JobSuite__Job__c);
        //if(selRow.JobSuite__Job_Task_Roles__r){
        ...
        if(selRow){
            ...
            $A.createComponent("forceChatter:publisher", {"context":"RECORD", "recordId":selRow.JobSuite__Job__c}, function(publisher) {
                var chatterPublisher = component.find("chatterPublisher");
                if(chatterPublisher){
                    chatterPublisher.set("v.body", publisher);
                }
            });
            $A.createComponent("forceChatter:feed", {"type":"Record", "feedDesign":"DEFAULT", "subjectId":selRow.JobSuite__Job__c}, function(feed) {
