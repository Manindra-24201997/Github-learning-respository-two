({
    ...
    onloadrecord : function(component, event, helper) {
        ...
        if(recId!=null){
            ...
            setTimeout($A.getCallback(function() {
                component.find("RoleId").set("v.value", CConFields.record.fields.JobSuite__Client_Contact_Role__c.value);
            }),1500);
        }
    },
    ...
    RecordSubmit: function(component, event, helper) {
       ...
       // CConFields["JobSuite__Client__c"]=component.get("v.simpleRecord.JobSuite__Client__c");
        var FirstName;
        var LastName;
        var missField = "";
        var missField1 = "";
        ...
        var reduceReutrn = component.find('CConField').reduce(function(validFields, inputCmp) {   
            if(inputCmp.get("v.fieldName") == "JobSuite__First_Name__c" ){
                FirstName = inputCmp.get("v.value");
                if (FirstName == null || FirstName == '') 
                    missField = "First Name";
            } 
            
            else if(inputCmp.get("v.fieldName") == "JobSuite__Last_Name__c"){
                LastName = inputCmp.get("v.value"); 
                if (LastName == null || LastName == '') 
                    missField1 = "Last Name";
            }
                else if(inputCmp.get("v.fieldName") == "JobSuite__Email__c"){
                    var Email = inputCmp.get("v.value"); 
                    if (Email == null || Email == '') 
                        missField2 = "Email";
                }
                    else if(inputCmp.get("v.fieldName") == "JobSuite__UserName__c	"){
                ...
                event.preventDefault();
                    ...
                    event.preventDefault();
                        ...
                        event.preventDefault();   
                            ...
                            CConFields["JobSuite__Client_Contact_Role__c"]=component.find("RoleId").get("v.value");
