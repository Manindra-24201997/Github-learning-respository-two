({
    getFieldLabels: function(component, event, helper) {
        ...
        Objaction.setParams({
            ObjNames : ['JobSuite__Shipment_Line_Item__c','JobSuite__Job__c']
        }); 
        Objaction.setCallback( helper, function( response ) {
            ...
            if(component.isValid() && response.getState() === 'SUCCESS' ) {
                ...
                component.set('v.columns', [
                    ...
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.Name.label, fieldName: 'ItemLink', sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'ItemName' }, target: '_self' ,tooltip:{ fieldName: 'ItemName' }}},
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Job_Name__c.label, fieldName: 'JobSuite__Job_Name__c', sortable: true, type: 'text'},
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Job__c.label, fieldName: 'JobSuite__Job__c',sortable: true, type: 'text'},
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Client_Name__c.label, fieldName: 'JobSuite__Client_Name__c',sortable: true, type: 'text'},
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Item_Status__c.label, fieldName: 'JobSuite__Item_Status__c',sortable: true, type: 'text'},
                    {label: tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Qty_Shipped__c.label, fieldName: 'JobSuite__Qty_Shipped__c',sortable: true, editable:'true',type: 'number',cellAttributes: {alignment: 'left' }}
                    
                ]); 
                var labels = {'Job':tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Job_Name__c.label,
                              'ClientName':tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Client_Name__c.label,
                              'Status':tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Item_Status__c.label,
                              'Shipped':tableHeader.JobSuite__Shipment_Line_Item__c.JobSuite__Qty_Shipped__c.label,
                              };
                ...
                component.set('v.ShpLicolumns', [
                    ...
                    {label: 'Quantity', fieldName: 'JobSuite__Quantity__c', type: 'number',cellAttributes: {alignment: 'left' }}
                ]);
        ...
        action1.setParams({
            sObjName :  "JobSuite__Shipment_Line_Item__c"
        });
            ...
            if(state === 'SUCCESS')
            {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    if(ShoWPE){
                        ...
                        if(row.JobSuite__Production_Estimate__c){
                            console.log('>>>>> If row PE Enter >>>>>>>>>>>>>>');
                            row.ItemLink='/'+row.JobSuite__Production_Estimate__c;
                            row.ItemName=row.JobSuite__Production_Estimate__r.Name;
                        }
                    }
                    else{
                        if(row.JobSuite__Estimate_Line_Item__c){
                            console.log('>>>>> If row Est Enter >>>>>>>>>>>>>>');
                            row.ItemLink='/'+row.JobSuite__Estimate_Line_Item__c;
                            row.ItemName=row.JobSuite__Estimate_Line_Item__r.Name;
                        }
                    }
                }
            }
        ...
        AddShpLI.setCallback(this, function(response){
            ...
            if(state === 'SUCCESS')
            {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    if(row.JobSuite__Job__c){
                        row.JobName=row.JobSuite__Job__r.Name;
                    }
                    
                    if(row.JobSuite__Job__r.JobSuite__JS_Client__c){
                        row.ClientName=row.JobSuite__Job__r.JobSuite__JS_Client__r.Name;
                    }
                }
