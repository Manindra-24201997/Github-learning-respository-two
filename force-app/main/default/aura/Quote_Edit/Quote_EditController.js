({
    ...
    onloadrecord : function(component, event, helper) {
        ...
        if (recId != null) {
            var quotefilds = event.getParam("recordUi");
             if(quotefilds.record.fields.JobSuite__GL_Department__c.value!=null){
              var GLDObj={"Id":quotefilds.record.fields.JobSuite__GL_Department__r.value.fields.Id.value, "Name":quotefilds.record.fields.JobSuite__GL_Department__r.value.fields.Name.value};
              var GLCObj={"Id":quotefilds.record.fields.JobSuite__GL_Code__r.value.fields.Id.value, "Name":quotefilds.record.fields.JobSuite__GL_Code__r.value.fields.Name.value};
              component.find("GLD_GLC_AuraId").sampleMethod(GLDObj,GLCObj);
            }
            
           // component.find("GLDSelectId").set("v.value", quotefilds.record.fields.JobSuite__GL_Department__c.value);
           // helper.getGLCode(component, event);
           /* window.setTimeout(
                $A.getCallback(function() {
                    component.find("GLCodeSelectId").set("v.value", quotefilds.record.JobSuite__fields.GL_Code__c.value);
                }), 500
            );*/
        }
    ...
    RecordSubmit: function(component, event, helper) {
        ...
        Quotefields["JobSuite__Job__c"] = component.get("v.jobrecordId");
        ...
        var reduceReutrn = component.find('JobQuoteField').reduce(function(validFields, inputCmp) {
            ...
            } else if (inputCmp.get("v.fieldName") == "JobSuite__Quantity__c") {
        ...
        if (missField == "Quote Name") {
            ...
            event.preventDefault();
             ...
             if(component.get("v.selectedLookUpRecord_GLD").Id==null){
                  Quotefields["JobSuite__GL_Department__c"]="";
              }
              else{
                  Quotefields["JobSuite__GL_Department__c"]=component.get("v.selectedLookUpRecord_GLD").Id;  
              }
              
              if(component.get("v.selectedLookUpRecord_GLC").Id==null){
                  Quotefields["JobSuite__GL_Code__c"]="";
              } 
              else{
                  Quotefields["JobSuite__GL_Code__c"]=component.get("v.selectedLookUpRecord_GLC").Id;
              }
