            ...
            if(res.getState() === "SUCCESS"){
                ...
                component.set("v.columns",[
                   ...
                   // { type: 'action', typeAttributes: {rowActions: rowActions}},
                    { label: tableHeaders.Approval_Job_Task__c.Project__c.label, fieldName: 'JobId', editable: true, sortable: true, type: 'url', typeAttributes: { label: { fieldName: 'JobName' }, target: '_blank' , tooltip:{ fieldName: 'JobName' }}},
                    { label: 'Approval Name', fieldName: 'TaskId', sortable: true, type: 'url',cellAttributes: {class: { fieldName: "TaskColorcode" } }, typeAttributes: { label: { fieldName: 'TaskName' }, target: '_parent', tooltip:{ fieldName: 'TaskName' }}},
            ...
            if (state === "SUCCESS") {
                if(cusSetVal){
                    ...
                    for (var i = 0; i < rows.length; i++) {
                        var row = rows[i];
                        console.log('>>>>>>>>>>>>'+JSON.stringify(row));
                        row.JobId= '/'+row.Project__c;
                        row.JobIdNo= '/'+row.Project__c;
                        row.TaskId= '/'+row.Id;
                        row.JobName=row.Project__r.Name;
                        row.TaskName=row.Name;
                        row.TaskDue=row.Revised_Due_Date__c;
                        row.FileName=row.File_Name__c;
                        row.JobNum=row.Project__r.Job_Auto_Number__c;
                    }
                    component.set("v.data",rows);
                }
