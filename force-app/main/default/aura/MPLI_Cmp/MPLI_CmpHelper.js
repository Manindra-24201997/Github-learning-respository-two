({
    ...
    deleteMOLIReceipt : function(component, event, helper,rcdid){ 
        var action = component.get("c.deleteMOLIReceiptrcd");
        action.setParams({
            rcdids : rcdid 
        });
        action.setCallback(this, function(response) {
            if (response.getState() === "SUCCESS") {
                ...
                if(response.getReturnValue() == 'deleted'){
                    helper.showToast({
                        "type":"success",
                        "message":  "MOLI Receipt Record Deleted Successfully."
                    }); 
    ...
    getRowActionsReceipt : function (component, row, doneCallback){
        ...
        var deleteAction = {
            ...
            'disabled' : row.MPLI_Receipt_Status__c == 'Received'
        };
