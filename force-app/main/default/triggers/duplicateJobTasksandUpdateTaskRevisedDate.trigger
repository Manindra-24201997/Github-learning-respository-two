    ...
    if(Trigger.IsAfter && Trigger.IsUpdate)
    {
        ...
        for(Job_Task__c objJT:Trigger.New)
        {
            ...
            If(!System.isFuture() && !system.isbatch())
            {
                ...
                if(Trigger.oldMap.get(objJT.Id).Next_Task_Due__c  != Trigger.NewMap.get(objJT.Id).Next_Task_Due__c ){
                    system.debug('Task is === '+objJT.Name);
                    MarkedDone=true;
                }
                jobids.add(sjob);  
            }
...
*/
    ...
    if(jobids.size()>0){
        ...
        if(CompletedTasks1.size() > 0 ){
            ...
            Job__c objJob=[Select id,name,Next_Tasks_Due__c,Job_Timing_Status__c,Due_Date__c from Job__c where Id=:sjob WITH SECURITY_ENFORCED limit 1];
